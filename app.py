...
# Wrap model loading to handle errors gracefully
try:
    supervised_pipeline, iforest_pipeline = load_models()
except Exception as e:
    st.error("Failed to load ML models. Proceeding with rule-based detection only.")
...
def score_transaction_ml(model_pipeline, iforest_pipeline, model_payload: Dict, convert_to_inr: bool = False, currency: str = "INR") -> Tuple[float, float, str]:
    ...
    """
    if model_pipeline is None or iforest_pipeline is None:
        return 0.0, 0.0, "LOW"  # Fallback if models not loaded
...
# STREAMLIT UI
# -------------------------
st.set_page_config(page_title="Real-Time Fraud Detection System", page_icon="🔒", layout="wide")
...
with st.container():
    ...
    with col0:
        currency = st.selectbox("Currency", CURRENCIES, index=CURRENCIES.index(DEFAULT_CURRENCY), key="currency_select", help="Select the transaction currency.")
    with col0b:
        st.caption(f"Exchange rate: 1 {currency} = {INR_PER_UNIT[currency]:,.2f} INR")
    col1, col2, col3 = st.columns(3)
    with col1:
        amount = st.number_input(f"Amount ({currency})", min_value=0.0, value=1200.0, step=10.0, key="amount_common", help="Enter the transaction amount.")
    with col2:
        txn_date = st.date_input("Date", value=datetime.date.today(), key="txn_date", help="Select the transaction date.")
    with col3:
        txn_time = st.time_input("Time", value=datetime.time(12, 0), key="txn_time", help="Select the transaction time.")
...
# Channel selector
channel = st.selectbox("Transaction Channel", ["Select Channel", "Bank", "Mobile App", "ATM", "Credit Card", "POS", "Online Purchase", "NetBanking"], key="channel_select", help="Choose the channel for the transaction.")

if channel and channel != "Select Channel":
    channel_lower = channel.lower()
    with st.container():
        st.header(f"{channel} Channel Configuration")
        # Transaction type options per channel (enforced)
        txn_options = CHANNEL_TXN_TYPES.get(channel_lower, ["OTHER"])
        txn_type = st.selectbox("Transaction Type", txn_options, key=f"txn_type_{channel_lower}", help="Select the type of transaction.")
    
    # Channel-specific fields
    with st.expander("Channel-Specific Parameters", expanded=True):
        if channel_lower == "bank":
            st.subheader("In-Branch Identification")
            ...
            atm_distance_km = st.number_input("Distance from Last Known Location (km)", min_value=0.0, value=0.0, step=1.0, key="atm_distance")
            card_masked = st.text_input("Masked Card Number (e.g., 4111****1111)", key="atm_card_masked")
        elif channel_lower == "mobile app":
            st.subheader("Mobile Application Details")
            ...
            app_version = st.text_input("Application Version", value="1.0.0", key="mobile_app_ver")
            last_device = st.text_input("Last Known Device (optional)", key="mobile_last_device")
        elif channel_lower == "credit card":
            st.subheader("Credit Card Mode")
            cc_mode = st.radio("Mode", ["POS (Physical)", "Mobile/Web"], key="cc_mode")
            if cc_mode == "POS (Physical)":
                card_masked = st.text_input("Masked Card Number (e.g., 4111****1111)", key="cc_pos_card")
                card_country = st.text_input("Card Issuing Country", key="cc_pos_country")
                cvv_provided = st.checkbox("CVV Provided", value=True, key="cc_pos_cvv")
                pos_merchant_id = st.text_input("POS Merchant ID (optional)", key="cc_pos_mid")
            else:
                card_masked = st.text_input("Masked Card Number (e.g., 4111****1111)", key="cc_web_card")
                card_country = st.text_input("Card Issuing Country", key="cc_web_country")
                cvv_provided = st.checkbox("CVV Provided", value=True, key="cc_web_cvv")
            ...
            st.subheader("Point of Sale Details")
            ...
            st.subheader("Online Purchase Details")
            ...
            billing_address = st.text_input("Billing Address", value="", key="online_bill")  # Removed default to shipping
            ...
            if used_card_online:
                cvv_provided = st.checkbox("CVV Provided", value=True, key="online_cvv")
                card_masked = st.text_input("Masked Card Number", key="online_card_masked")
            ...
            st.subheader("Net Banking Details")
            username = st.text_input("User ID/Login", key="nb_user")
            device = st.text_input("Device/Browser", key="nb_device")
            ...
            new_beneficiary = st.checkbox("Newly Added Beneficiary?", value=False, key="nb_new_benef")
            beneficiary_added_minutes = st.number_input("Minutes Since Beneficiary Added", min_value=0, value=9999, step=1, key="nb_benef_minutes")

    # Optional telemetry
    with st.expander("Customer Telemetry (Optional)", expanded=False):
        st.subheader("Spending and Activity Metrics")
        colT1, colT2 = st.columns(2)
        with colT1:
            monthly_avg = st.number_input(f"Monthly Average Spend ({currency})", min_value=0.0, value=10000.0, step=100.0, key=f"monthly_avg_{channel_lower}")
        ...
        with colT2:
            ...
            beneficiaries_added_24h = st.number_input("Beneficiaries Added in Last 24 Hours", min_value=0, value=0, step=1, key=f"ben24_{channel_lower}")
            failed_login_attempts = st.number_input("Recent Failed Login Attempts", min_value=0, value=0, step=1, key=f"failed_{channel_lower}")

    # IP & Geo telemetry
    with st.expander("IP and Geolocation Data (Optional)", expanded=False):
        if channel_lower == "bank":
            st.info("IP and client data not collected for in-branch transactions.")
            client_ip = ""
            ip_country = ""
            ...
            ip_country = st.text_input("IP-Derived Country", key=f"ip_country_{channel_lower}")
            suspicious_ip_flag = st.checkbox("IP Flagged by Threat Intelligence?", value=False, key=f"suspicious_{channel_lower}")
            colG1, colG2 = st.columns(2)
            with colG1:
                last_known_lat = st.number_input("Last Known Latitude", format="%.6f", value=0.0, key=f"lastlat_{channel_lower}")
                last_known_lon = st.number_input("Last Known Longitude", format="%.6f", value=0.0, key=f"lastlon_{channel_lower}")
            with colG2:
                txn_lat = st.number_input("Transaction Latitude", format="%.6f", value=0.0, key=f"txnlat_{channel_lower}")
                txn_lon = st.number_input("Transaction Longitude", format="%.6f", value=0.0, key=f"txnon_{channel_lower}")
    ...
    # Submit
    submit = st.button("Analyze Transaction", key=f"submit_{channel_lower}", type="primary")
    ...
    if submit:
        ...
        with st.spinner("Performing fraud analysis..."):
        ...
        badge_color = color_map.get(final_risk, "#6b7280")
        st.markdown(
            f"""<div style="padding:1rem;border-radius:6px;background-color:{badge_color}10;border:1px solid {badge_color};text-align:center;">
                <strong style="color:{badge_color};font-size:1.25rem;">Overall Risk Level: {final_risk}</strong>
            </div>""",
            unsafe_allow_html=True,
        )
        colA, colB = st.columns(2)
        with colA:
            st.metric("Fraud Probability (ML)", f"{fraud_pct:.2f}%", help="Probability from supervised machine learning model.")
            st.metric("ML Risk Assessment", ml_label)
        with colB:
            st.metric("Anomaly Score (ML)", f"{anomaly_pct:.2f}%", help="Anomaly detection score from isolation forest.")
            st.metric("Rules-Based Severity", rules_highest)
        st.subheader("Triggered Rules")
        if rules_triggered:
            for r in rules_triggered:
                sev_color = color_map.get(r["severity"], "#6b7280")
                st.markdown(f"**{r['name']}** - <span style='color:{sev_color};'>{r['severity']}</span>", unsafe_allow_html=True)
                st.caption(r["detail"])
        else:
            st.success("No rules were triggered.")
        with st.expander("Debug: Input Payload", expanded=False):
            st.json(payload)
else:
    st.info("Please select a currency, enter the amount, date, and time, then choose a transaction channel to proceed.")
